...
func uploadTestAssets(configs ConfigsModel, log log.Logger) (TestAssetsAndroid, error) {
	...
	err = uploadFile(testAssets.testApp.UploadURL, configs.AppPath, log)
	if err != nil {
	...
	if configs.TestType == testTypeInstrumentation {
		if err := uploadFile(testAssets.TestApk.UploadURL, configs.TestApkPath, log); err != nil {
	...
	if configs.TestType == testTypeRobo && configs.RoboScenarioFile != "" {
		if err := uploadFile(testAssets.RoboScript.UploadURL, configs.RoboScenarioFile, log); err != nil {
	...
	for i, obbFile := range configs.ObbFiles {
		if err := uploadFile(testAssets.ObbFiles[i].UploadURL, obbFile, log); err != nil {
...
func startTestRun(configs ConfigsModel, testAssets TestAssetsAndroid, log log.Logger) error {
